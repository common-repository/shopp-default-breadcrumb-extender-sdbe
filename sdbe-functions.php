...
function sdbe( $result, $options, $Catalog ){
	...
	$in_category = 	shopp('collection.get-id');
	
	if( !(  shopp( 'storefront.get-type' ) == 'product' ) ){ 
	...
	if( ( shopp( 'product.in-category',"id=$in_category" ) ) ){
		...
		if( shopp( 'product', 'has-categories' ) ){
			...
			while( shopp( 'product', 'categories' ) && empty( $sdbe_cat_id ) ){
				$sdbe_cat_id = shopp( 'product.get-category', 'show=id' );
			}
...
function sdbe_generate_cat_tree(){
	...
	if( shopp( 'storefront', 'has-categories' ) ){
		while( shopp( 'storefront', 'categories' ) ){
				$key = shopp( 'collection.get-id' );
				$tree[ $key ]['name'] = shopp( 'collection.get-name' );
				$tree[ $key ]['url'] = shopp( 'collection.get-url' );
				if( shopp( 'collection', 'is-subcategory' ) ) {
					$tree[ $key ]['parent'] = shopp( 'collection.get-parent','show=id' );
				} else {
					$tree[ $key ]['parent'] = '';				
				}
		}	
